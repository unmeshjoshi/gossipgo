...
func ExampleNewGoSchema() {
	...
	if err != nil {
		log.Printf("failed building schema: %s", err)
	}

	yaml, err := s.ToYAML()
	if err != nil {
		log.Printf("failed converting to yaml: %s", err)
	}
...
func ExampleToYAML() {
	...
	if err != nil {
		log.Printf("failed building schema: %v", err)
	}

	yaml, err := s.ToYAML()
	if err != nil {
		log.Printf("failed converting to yaml: %v", err)
	}
	fmt.Println(string(yaml))
	// Output:
	// db: Test
...
func TestYAMLRoundTrip(t *testing.T) {
	...
	if err != nil {
		log.Printf("failed to convert from yaml to a schema: %v", err)
	}
	if !reflect.DeepEqual(s, s2) {
		log.Print("yaml round trip schemas differ")
	}
}
